...
/**
 * \fn spi* yo_spi_init (uint8_t spidev_device, uint8_t chip_select)
 ...
 */
spi* yo_spi_init (int spidev_device, uint8_t chip_select);

/**
 * \fn int yo_spi_free(spi* spi)
 * \brief frees the malloced spi struct
 * \param spi* spi - valid spi struct pointer
 * \return EXIT_SUCCESS or EXIT_FAILURE
 */
int yo_spi_free(spi* spi);

/**
 * \fn int yo_spi_set_mode(spi* spi, spi_mode mode)
 ...
 * \return EXIT_SUCCESS or EXIT_FAILURE
 */
int yo_spi_set_mode(spi* spi, spi_mode mode);

/**
 * \fn int yo_spi_set_bits_per_word(spi* spi, spi_bpw bpw)
 * \brief sets the bits per word of the spi transfer, either 8 or 16
 * \param spi* spi - valid spi struct pointer
 * \param enum spi_bpw - bits per word eith BITS_8 or BITS_16
 * \return EXIT_SUCCESS or EXIT_FAILURE
 */
int yo_spi_set_bits_per_word(spi* spi, spi_bpw bpw);

/**
 * \fn int yo_spi_set_speed(spi* spi, uint32_t speed)
 ...
 * \return EXIT_SUCCESS or EXIT_FAILURE
 */
int yo_spi_set_speed(spi* spi, uint32_t speed);

/**
 * \fn spi_mode yo_spi_get_mode(spi* spi)
 * \brief gets the current mode of the spi bus
 * \param spi* spi - valid spi struct pointer
 * \return enum spi_mode - MODE_0/1/2/3 on success, MODE_ERROR on fail
 */
spi_mode yo_spi_get_mode(spi* spi);

/**
 * \fn uint32_t yo_spi_get_speed(spi* spi)
 * \brief gets the current speed of the spi bus
 * \param spi* spi - valid spi struct pointer
 * \return uint32 - current speed of spi bus in Hz
 */
uint32_t yo_spi_get_speed(spi* spi);

/**
 * \fn spi_bpw yo_spi_get_bits_per_word(spi* spi)
 * \brief gets the current bits per word of the spi bus
 * \param spi* spi - valid spi struct pointer
 * \return enum spi_bpw - BITS_8/16 on success, BITS_ERROR on fail
 */
spi_bpw yo_spi_get_bits_per_word(spi* spi);

/**
 * \fn int yo_spi_write(spi* spi, uint8_t* tx, uint32_t len)
 ...
 * \return EXIT_SUCCESS or EXIT_FAILURE
 */
int yo_spi_write(spi* spi, uint8_t* tx, uint32_t len);

/**
 * \fn int yo_spi_read(spi* spi, uint8_t* rx, uint32_t len)
 ...
 * \return EXIT_SUCCESS or EXIT_FAILURE
 */
int yo_spi_read(spi* spi, uint8_t* rx, uint32_t len);

/**
 * \fn int yo_spi_rw(spi* spi, uint8_t* tx, uint8_t* rx, uint32_t len)
 ...
 * \return EXIT_SUCCESS or EXIT_FAILURE
 */
int yo_spi_rw(spi* spi, uint8_t* tx, uint8_t* rx, uint32_t len);

//debug
void yo_debug(const char *func, char *format, ...) __attribute__((format(printf, 2, 3)));
void yo_warn(const char *format, ...) __attribute__((format(printf, 1, 2)));
int yo_get_debug();
void yo_set_debug(int level);
